...
// g++-9 -std=c++11 -fopenmp -O2 -o parareal-kdv parareal-kdv.cpp && ./parareal-kdv.cpp
//
//     If N = 128, need dt = 1e-2 for SBDF2 stability
//     If N = 256, need dt = 4e-4 for SBDF2 stability
//     If N = 512, need dt = 1e-5 for SBDF2 stability
//
#include <stdio.h>
#include <math.h>
#include <cstdlib>
...
void SBDF2_kdv(dcomp* y, double t0, double tfinal,
    ...
    */
    ...
    for(int m = 0; m < M-1; m++) {
      ...
      // printf("SBDF2 Iteration %d/%d\r", m+1,M);
...
void parareal(dcomp* u, //solution to write to, size (M+1) x N
  ...
  */
  ...
  {//begin parallel region
    ...
    // dcomp* um1_buff = (dcomp*) malloc(mem_size); //buffer for um1, has IC
    dcomp* B_buff = (dcomp*) malloc(mem_size); // to hold B for TRBDF2
    dcomp* Bm1_buff = (dcomp*) malloc(mem_size); // to hold Bm1 for TRBDF2
    // dcomp* fft_buff  = (dcomp*) malloc(mem_size); //buffer for the fft in kdv_rhs_hat
...
int main(int argc, char** argv){
  ...
  */
  ...
  for(long s=0; s<M; s++){
    for(long j=0; j<N; j++) u_hat0[j] = u_hat_coarse[j+s*N]; //copy correct IC to a buffer u_hat0
  ...
  for(long s=0; s<M; s++){
    for(long j=0; j<N; j++) u_hat0[j] = u_hat_fine[j+s*N]; //copy correct IC to a buffer u_hat0
